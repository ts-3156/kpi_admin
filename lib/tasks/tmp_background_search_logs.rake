namespace :tmp_background_search_logs do
  desc 'create'
  task create: :environment do
    ActiveRecord::Base.connection.execute('DROP TABLE IF EXISTS tmp_background_search_logs')
  ...
  task import: :environment do
    ...
    1000.times do |n|
      ...
      sql = ActiveRecord::Base.send(:sanitize_sql_array, [import_tmp_background_search_logs_sql, start: day.beginning_of_day, end: day.end_of_day])
      ActiveRecord::Base.connection.execute(sql)
    end
  end
end

def import_tmp_background_search_logs_sql
  <<-"SQL".strip_heredoc
      INSERT INTO tmp_background_search_logs
      SELECT
        a.id,
        a.session_id,
        a.user_id,
